...
const WithdrawPage = (props: {
  ...
  const [hcashToken, setHcashToken] = useState("");
    ...
    if (ws === WITHDRAWALSTATES.Nothing) {
      ...
      return (
        <div>
          <Box>
            ...
            <Text>Your Fantom is now ready to withdraw.</Text>
          </Box>
        </div>
      );
      ...
      return (
        <div>
          <Box>
            ...
            <Text>
              To get your FTM, you'll need to wait for more participants to
              join.{" "}
            </Text>
            <Flash my={3}>
              The pool currently has {ringParticipants.deposited}/
              {ringParticipants.participantsRequired} FTM participants(s).
            </Flash>
      ...
      return (
        <div>
          <Box>
            ...
            <Text>
              There's not enough Fantom in this pool right now to make your
      ...
      return (
        <Box>
          ...
          <Text>There's no Fantom in the pool for:</Text>
          ...
          <Text>Try changing your Wallet account.</Text>
        </Box>
      );
      ...
      return (
        <Box>
          ...
          <Text>This token's already been used to withdraw FTM.</Text>
        </Box>
      );
        ...
        <div>
          <Box justifyContent="center">
            ...
            <Button.Text
              textAlign="center"
              as="a"
              href={`https://testnet.ftmscan.com/tx/${txReceipt.transactionHash}`}
            >
              View on etherscan
            </Button.Text>
          </Box>
      ...
      return (
        <Box>
          ...
          <Text>
            ...
            <a href="">setup your own relayer</a>
          </Text>
        </Box>
      );
    }
  ...
  return (
    <Box style={{ width: "100%" }} mb={6}>
      <Form
        onSubmit={(e) => {
          (async () => {
            ...
            // Invalid hcashToken
            if (hcashToken.split("-").length - 1 !== 3) {
              setWithdrawalState(WITHDRAWALSTATES.CorruptedToken);
              return;
            }

            // eslint-disable-next-line no-unused-vars
            const [ethAmount, ringIdx, randomSk] = hcashToken
              .split("-")
              .slice(1);

            // Checks if ring is closed
            const ringHash = await hcashswapInstance.methods
            ...
            if (ringHash.length !== 66) {
              const participantsRequired = await hcashswapInstance.methods
                .getRingMaxParticipants()
                .call();

              const curParticipants = await hcashswapInstance.methods
            ...
            const publicKeys: [[String, String]] =
              await hcashswapInstance.methods
            ...
            const dataBytecode = hcashswapInstance.methods
              .withdraw(ethAddress, ethAmount, ringIdx, c0, keyImage, s)
              .encodeABI();

            if (useRelayer) {
              const relayerURL = useDefaultRelayer ? "" : customerRelayerURL;
              try {
                ...
                const message = `Get Fantom from Hcashswap via Relayer (Destination: ${dappGateway.ethAddress})`;
      ...
      >
        <Card>
          <Heading.h3 fontSize="3">Get FTM</Heading.h3>
          <Text my="3">
            Paste your Hcash token to see if your FTM is ready to withdraw
            privately.
          </Text>

          <Form.Field label="Hcash token" width={1}>
            <Form.Input
              type="text"
              placeholder="e.g. hcash-x-xx-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
              required
              width={1}
              value={hcashToken}
              onChange={(e) => setHcashToken(e.target.value)}
            />
          </Form.Field>
          <Box textAlign="right">
            <Tooltip
              message="Whoever deposited your FTM should have your Hcash token"
              placement="bottom"
            >
            ...
            {useRelayer ? (
              <div>
                <Field label="Select your relayer">
                  <div>
                    <Radio
                      label="Default - relayer.hurricash.com"
                      my="1"
                ...
                <Flex alignItems="center" my="3">
                  ...
                  <Box>
                    <Text italic>
                      You will pay a small amount of fees to the relayer for GAS
                      purposes.
                    </Text>
                  </Box>
                </Flex>
              ...
              <Flex alignItems="center" my="3">
                ...
                <Box>
                  <Text italic>
                    You will need some FTM in your withdrawal address to pay
                    for GAS.
                  </Text>
                </Box>
              </Flex>
